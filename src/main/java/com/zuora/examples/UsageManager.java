...
import com.zuora.sdk.utils.ThreadUtils;
...
public class UsageManager {
    ...
    static final String SAMPLE_USAGE_FILE = "/home/user/NetBeansProjects/zuora-rest-java/src/com/zuora/sdk/examples/sample_usage.csv";
    ...
    public void create(String fullyQualifiedFilename) {
        ...
        for (int i = 0; i < EXTENSION.length; i++) {
            if (extension.toLowerCase().equals(EXTENSION[i])) {
                break;
            }
            if (i == 2) {
        ...
        if (response != null && (Integer) response.get("httpStatusCode") == 200 &&
                (Boolean) response.get("success") && response.get("checkImportStatus") != null) {

            // hang in there a bit and see if import worked
            ThreadUtils.sleep();

            args = new ZAPIArgs();
            args.set("uri", response.get("checkImportStatus"));
